...
class AcademicContentService{
     ...
     */
    public function addEntry(int $idCourseDescription, string $academicContent) : bool {
        try{
            if(!is_numeric($idCourseDescription) || $idCourseDescription < 1){
                $this->errorMsgs = array("\$idCourseDescription: " . $idCourseDescription . ": invalid number");
                return false;
            ...
            if(!is_null($academicContent) || !empty($academicContent)){
                $academicContent = substr($academicContent, 0, 1800) ;
            } else {
                $this->errorMsgs[] = "\$academicContent cannot be empty or null!";
                return false;
            }
